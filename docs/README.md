...
2. Go to the project directory `cd /var/www/INSIGHTS/frontend`
...
1. Access the server `172.16.0.115` via SSH 
2. Go to the project directory `cd /var/www/INSIGHTS/frontend`
...
- The main entry point is the `main.jsx` file.
- The `/images` directory contains images used throughout the application.
- The `/components` directory is organized as follows:
  - `/common`: Contains common components used throughout the application, such as `NavBar`, `Footer`, and menus.
  - `/layout`: Contains layout-specific components, such as `root.jsx`.
  - `/pages`: Contains the main pages of the application, organized further into subdirectories for specific features:
    - `/payslips`: Components related to payslips, such as `Payslips.jsx` and `PayslipsPreview.jsx`.
    - `/vacancies`: Components for vacancies, such as `Vacancies.jsx`.
    - `/vacations`: Components for vacation requests and management, such as `VacationsRequest.jsx`.
  - `/shared`: Contains shared components used across multiple pages, including carousels, sliders, and detectors.
    - `/embla-carousel`: Specialized carousel-related components, such as `EmblaCarousel.jsx`.
- The `/assets` directory contains utility files like `handleError.js` and `getApi.js`.
- The `/contexts` directory contains context files for managing app-wide state, such as `SnackbarContext.jsx` and `ProgressbarContext.jsx`.
- The `/routes` directory manages routing-related configurations and components.
- The `/theme` directory contains files related to styling and theming.
- Additional files and scripts:
  - `convert_to_webp.sh`: A script for converting images to the WebP format.
  - `setupTests.js`: A setup file for testing.
...
const ShoppingCart = ({ initialItems }) => {
	...
	return (
		<div>
			...
			<button onClick={() => addItem({ name: "NewItem", price: 10 })}>Add Item</button>
		</div>
	);
};
