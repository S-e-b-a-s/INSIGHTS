...
const Vacancies = () => {
    ...
    const getVacancies = async () => {
        try {
            const response = await fetch(
                `${getApiUrl().apiUrl}vacancy/vacancy/`,
                {
                    method: 'GET',
                    credentials: 'include',
                }
    ...
    const submitReferVacancy = async () => {
        ...
        try {
            ...
            if (response.status === 201) {
                showSnack(
                    'success',
                    'La información de la vacante ha sido enviada correctamente.'
                );
            }
            setOpenCollapse(false);
            setOpenVacancy(false);
    ...
    const submitApplyVacancy = async () => {
        ...
        try {
            const response = await fetch(
                `${getApiUrl().apiUrl}vacancy/apply/`,
                {
                    method: 'POST',
                    body: formData,
                    credentials: 'include',
            ...
            if (response.status === 200) {
                showSnack(
                    'success',
                    'Se ha enviado tu postulación correctamente.'
                );
    ...
    const handleInactiveVacancy = async () => {
        try {
            const response = await fetch(
                `${getApiUrl().apiUrl}vacancy/vacancy/${inactivateId}/`,
                {
                    method: 'PATCH',
                    credentials: 'include',
            ...
            if (response.status === 200) {
                ...
                showSnack(
                    'success',
                    'La vacante ha sido inactivada correctamente.'
                );
    ...
    return (
        <>
            ...
            >
                ...
                <Box>
                    <Typography color="gray">
                        Si deseas aplicar o referir a una de nuestras de
                        vacantes disponibles selecciona una de ellas y sigue los
                        pasos
                    </Typography>
                </Box>
                <Box
            ...
            <Dialog
                open={openAddVacancy}
                onClose={handleCloseAddVacancy}
                aria-labelledby="alert-dialog-title"
                aria-describedby="alert-dialog-description"
            >
                <DialogTitle id="alert-dialog-title">
                    {'¿Desea añadir una nueva vacante?'}
                </DialogTitle>

                <Box
                    component="form"
                    onSubmit={submitAddVacancy}
                    sx={{
                        flexDirection: 'column',
                        p: '1rem 1.5rem ',
                        display: 'flex',
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', gap: '2rem' }}>
                        <Button onClick={handleCloseAddVacancy}>
                            Cancelar
                        </Button>
            ...
            >
                ...
                <Collapse in={openCollapse}>
                    ...
                    >
                        ...
                        <TextField
                            ...
                            sx={{ width: '400px', mb: '1rem' }}
                            label="Numero de celular del referido"
                        ></TextField>
                        <Box sx={{ display: 'flex', gap: '2rem' }}>
                            <Button onClick={handleCloseCollapse}>
                                Cancelar
                            </Button>
                ...
                <Collapse in={!openCollapse}>
                    <DialogActions>
                        ...
                        <Button onClick={handleOpenCollapse}>
                            Referir a un conocido
                        </Button>
                        {localStorage.getItem(`vacancy${vacancyId}`) ==
                        vacancyId ? (
                            <Button disabled>
                                Ya has aplicado a esta vacante
                            </Button>
                        ) : (
                            <Button onClick={submitApplyVacancy}>
                                Aplicar
                            </Button>
                        )}
            ...
            <Dialog
                open={openDialogInactivate}
                onClose={handleCloseInactiveDialog}
                aria-labelledby="alert-dialog-title"
            >
                <DialogTitle id="alert-dialog-title">
                    {'¿Desea inactivar esta vacante?'}
                </DialogTitle>
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        La vacante será desactivada y ya no estará disponible,
                        sin embargo, las referencias relacionadas con ella no se
                        perderán.
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
                    <Button onClick={handleCloseInactiveDialog}>
                        Cancelar
                    </Button>
